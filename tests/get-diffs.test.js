...
test("Get Session Diff", async (t) => {
    ...
    await db.prepare('INSERT INTO session_state (short_id, udt_json, patch, version) VALUES (?, ?, ?, ?)')
        .run(secondObjectToInsert.short_id,
    ...
    db.prepare(`DELETE FROM session_state WHERE short_id = ?`).run(objectToInsert.short_id)
})
