...
test("Patch Session", async (t) => {
    ...
    await db.prepare('INSERT INTO session_state (short_id, udt_json) VALUES (?, ?)')
        .run(objectToInsert.short_id, JSON.stringify(objectToInsert.udt_json));

    const firstState = await db.prepare(`SELECT  *
                           FROM latest_session_state
                           WHERE short_id = ?
                           LIMIT 1`).get(objectToInsert.short_id)
    t.assert(firstState)
    ...
    const secondState = await db.prepare(`SELECT  *
                           FROM latest_session_state
                           WHERE short_id = ?
                           LIMIT 1`).get(objectToInsert.short_id)
    ...
    await db.prepare(`DELETE FROM session_state WHERE short_id = ?`).run(objectToInsert.short_id)
})
...
test("Patch Delete Sample", async (t) => {
    ...
    objectToInsert.udt_json.taskData.forEach((sample, index) => {
        samplesQueries.push(db.prepare('INSERT INTO sample_state (session_short_id, session_sample_id, content) VALUES (?, ?, ?)').run(objectToInsert.short_id, index, JSON.stringify(sample)));
    })
    ...
    const getSessionResponse = await getJSON(`${url}/api/session/${objectToInsert.short_id}`)
    ...
    await db.prepare(`DELETE FROM session_state WHERE short_id = ?`).run(objectToInsert.short_id)
    await db.prepare(`DELETE FROM sample_state WHERE session_short_id = ?`).run(objectToInsert.short_id)
})